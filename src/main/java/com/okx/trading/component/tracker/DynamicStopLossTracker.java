...
import com.okx.trading.component.tracker.dto.PriceUpdate;
import com.okx.trading.component.tracker.dto.StopLossAdjustment;
import lombok.*;
...
public class DynamicStopLossTracker {
    ...
    private static final Logger strategyLogger = LoggerFactory.getLogger("wss.strategy.msg");
     ...
     */
    public synchronized boolean updateData(@NonNull BigDecimal newPrice) {
        if (newPrice.compareTo(BigDecimal.ZERO) <= 0) {
            strategyLogger.error("价格更新失败: 价格必须大于0, 输入值: {}", newPrice);
        ...
        strategyLogger.info("\n===== 第{}次价格更新: {} =====", priceHistory.size(), JSON.toJSONString(priceHistory.get(priceHistory.size() - 1)));
     ...
     * 获取统计信息
     */
    public com.okx.trading.component.tracker.TradeStatistics getStatistics() {
        int size = priceHistory.size();
        int start = Math.max(0, size - 10);
        List<BigDecimal> newPriceHistory = priceHistory.subList(start, size)
                .stream()
                .map(PriceUpdate::getPrice)
                .collect(Collectors.toList());

        strategyLogger.info("更新价格{}", JSON.toJSONString(newPriceHistory));

        return com.okx.trading.component.tracker.TradeStatistics.builder()
                .startTime(startTime)
    ...
    public void logAdjustmentHistory() {
        ...
        strategyLogger.info(historyLog.toString());
    }
}

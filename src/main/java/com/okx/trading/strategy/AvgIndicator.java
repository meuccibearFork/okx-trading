...
public class AvgIndicator extends CachedIndicator<Num> {
    ...
    public Num calculate(int index) {
        ...
        Num num = range.dividedBy(Ta4jNumUtil.valueOf(4));

        log.info("index:{} h:{} l:{} c:{} o:{} range:{}", index, h, l, c, o, num);
        return num;
    }

    public int compareTo(int index, int prevIndex) {
        log.info("AvgIndicator.compareTo:index:{} prevIndex:{}", index, prevIndex);
        return calculate(index).compareTo(calculate(prevIndex));
    }
